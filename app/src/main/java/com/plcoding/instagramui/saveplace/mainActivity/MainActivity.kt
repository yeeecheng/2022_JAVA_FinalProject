...
class MainActivity : AppCompatActivity(), SensorEventListener  {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //setting data base
        database = ContactDatabase.invoke(this)
...
//            var listOtpKeyIv: List<String> = otpKeyIv.split("?")
    ...
    private fun storeData(data:String){
        ...
        while(index <= addNum){
            ...
            item.uid =dataContent[0].toInt()
            item.name =dataContent[1]
            item.type =dataContent[2]
            item.information =dataContent[3]
            item.lng =dataContent[4].toDouble()
            item.lat =dataContent[5].toDouble()
            item.time_start =dataContent[6].toInt()
            item.time_end =dataContent[7].toInt()
...
//
    ...
    private fun passKeyAndIvToEmergency(phone: String, key: String, initVector: String): String{ //key inivector
        val aes = AESCrypt()
        var phone_16byte = aes.toHex((phone + "000000").toByteArray())
